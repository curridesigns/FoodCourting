//this file is the physics engine for the miniGame
...
//affects the objects pos to make it move
physics.movement = function(obj) {
  ...
  //slows the object down based to a stop
  if (obj.brakePedal.y) {
    obj.direction.y = 0;
  ...
  //makes the object unable to move faster than its "speedlimit"
...
//tells if something has hit something on the screen that it can't move through
physics.collision = function(obj, contact) {
  //these are the pixel values for the counters in the kitchen
  const counters = {
    front: { left: 500, bottom: 900 },
    top: { left: 655, bottom: 165 },
    island: { top: 430, bottom: 675, left: 655, right: 1720 },
    bottom: { top: 900, left: 655 }
  };
...
//this is a hold over from before the images were added, but it still works to make sure nothing leaves the screen if any of collision() fails
physics.worldSpace = function(obj, contact) {
  const worldBoundries = {
    y: { upper: 5, lower: canvas.height - 5 },
    x: { left: 310, right: canvas.width - 46 }
  };
